    ...
    def __init__(self, args):
        ...
        for k, d in splits.items():
            if args.split_id in k:
                for ind, task in enumerate(d[args.start_task_id:args.end_task_id]):
                    # load json file
    ...
    for batch_idx, batch in enumerate(
        ...
        for meta, step_id, image, annotations in batch:
            ...
            if annotations:
                ...
                if ((batch_idx == len(loader)-1)) and batch_idx > 0:

                    metadata_file = os.path.join(args.images_folder, args.split_id, "metadata_" + str(batch_idx) + ".json")

                    with open(metadata_file, mode="w+") as out_file:
                        json.dump(all_data, out_file)
                    # zip content

                    out_zip = f"{args.images_folder}/{args.split_id}_zip/task_{args.start_task_id}-{args.end_task_id}"
                    input_folder = f"{args.images_folder}/{args.split_id}"
                    shutil.make_archive(out_zip, 'zip', input_folder)

                    # remove content from train
                    del_content = os.listdir(input_folder)
                    print(del_content)

                    for f in del_content:
                        del_path = os.path.join(input_folder, f)
